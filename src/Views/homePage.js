...
const HomePage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await request.get(`/MedicalStaff/exception/${staffId}`);
    ...
    return (
        <div className={"container"}>
            <WorkBench />
            <div className={"alert"}>
                <Statistics data={{ all: alertData.length, list: countTypes }}></Statistics>
                {alertData.map((data, index) => (
                    <ElderlyAlertCard key={index} data={data}></ElderlyAlertCard>
                ))}
            </div>
            <div className={"charts"}>
                <RoomAlertChart data={alertData} roomNumber={roomNumber} targetDate={targetDate}/>
