...
import { Breadcrumb, Layout, Menu, theme, useLocation } from "antd";
import "./App.scss";
import {
...
const App = () => {
  const location = useLocation(); // This hook gives us the current location

  // Determine if the layout should be minimal (for login page)
      ...
      {!isLoginPage && (
        <Layout className="container">
          ...
          <Content className={isLoginPage ? "full-screen" : "content"}>
            <div className={isLoginPage ? "" : "content-inner"}>
              <Routes>
          ...
          {!isLoginPage && (
            <Footer className="footer">
              Group of Barbie ©{new Date().getFullYear()} Created by FrontEnd
              Team of Barbie
            </Footer>
          )}
        </Layout>
