  ...
  useEffect(() => {
    const fetchHealthDataForAll = () => {
      if (residentsInfo.length > 0) {
        const requests = residentsInfo.map((resident) => {
          ...
          return fetch(url, {
            ...
            .then((data) => {
              if (data.code === 200) {
                return { id: resident.residentId, data: data.data };
              } else {
        ...
        Promise.all(requests)
          .then((results) => {
            const updatedHealthData = results.reduce((acc, { id, data }) => {
              acc[id] = data;
              return acc;
            }, {});
            setResidentsHealthData(updatedHealthData);
          })
          .catch((error) => {
            notification.error({
              message: "请求错误",
    ...
    // Clean-up function
    return () => clearInterval(intervalId);
  }, [residentsInfo]); // Ensure useEffect is called whenever residentsInfo changes
