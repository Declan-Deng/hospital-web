      ...
      const fetchHealthDataForAll = () => {
        const healthDataPromises = residentsInfo.map((resident) => {
          ...
          return fetch(url, {
            ...
            .then((data) => {
              if (data.code === 200) {
                return { [resident.residentId]: data.data };
              } else {
        ...
        Promise.all(healthDataPromises)
          .then((results) => {
            const updatedHealthData = results.reduce(
              (acc, current) => ({
                ...acc,
                ...current,
              }),
