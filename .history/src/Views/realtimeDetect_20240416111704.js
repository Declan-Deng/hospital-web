...
const RealtimeDetect = () => {
  ...
  useEffect(() => {
    ...
    // 使用 POST 方法发送请求
    fetch(
      `https://n58mgwvs5a83.hk1.xiaomiqiu123.top/RoomData/getRoomEnvironment/${staffId}`,
      {
        method: "POST", // 指定请求方法为 POST
        headers: {
          "Content-Type": "application/json", // 设置请求头部，声明发送的数据类型为 JSON
        },
        body: JSON.stringify({ staffId: staffId }), // 发送的数据体，根据后端要求调整
      }
    )
      .then((response) => {
        if (!response.ok) {
