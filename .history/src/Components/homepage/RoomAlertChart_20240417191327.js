...
const RoomAlertChart = ({ data, roomNumber, targetDate }) => {
    ...
    // 获取昨天的日期字符串
    const yesterday = formatDate(new Date(targetDate.getFullYear(), targetDate.getMonth(), targetDate.getDate() - 1));

    // 数据转换：按小时统计
    const hourlyData = useMemo(() => {
        const hourlyCounts = Array(24).fill(0); // 创建一个长度为24，初始值为0的数组
        data.forEach(item => {
            if (item.roomNumber === roomNumber && formatDate(item.exceptionStartTime) === yesterday) {
    ...
    return (
        <ReactECharts option={getOption()} style={{ height: 400, width: '60%', display:"inline-flex" }} />
    );
};
