...
const Statistics = ({ data }) => {
  // 假设每个警报项是一个对象，包含isCurrent属性
  // 过滤出历史警报（isCurrent为false的项目）
  const filteredData = Object.entries(data.list)
    .filter(([key, value]) => !value.isCurrent)
  ...
  return (
    <div className="scontainer">
      <div className="left">
        <div className="left-header">当前共有历史报警</div>
      ...
      <div className="right">
        {Object.entries(filteredData).map(([key, value], index) => {
          // 计算当前值占总和的百分比
          const widthPercent = ((value.count / sum) * 100).toFixed(2); // 保留两位小数
          return (
            <div key={index} className="skill-box">
              ...
              <div className="skill-bar">
                ...
                >
                  <span className="tooltip">{`${value.count}条 (占${widthPercent}%)`}</span>
                </span>
              </div>
            </div>
          );
